use crate::{
    BuilderParams, TestCaseBuilder, TestCaseKind,
    filler::{default_bytecode_builder, ensure_memory_input_size_builder},
};
...
pub(super) fn fill(map: &mut BTreeMap<OpCode, Arc<TestCaseBuilder>>) {
    const MAX_MEMORY_SIZE_LOG2: usize = 21;

    let memory_filler = |memory: &mut SharedMemory, params: BuilderParams| {
        let mut rng = params.rng();
        let size = rng.random_range(0..MAX_MEMORY_SIZE_LOG2.pow(2));
        memory.resize(size);
        ...
        Arc::new(TestCaseBuilder {
            ...
            stack_builder: Box::new(|stack, params| {
                for _ in 1..=params.repetition {
                    ...
                    assert!(stack.push(U256::from((MAX_MEMORY_SIZE_LOG2 - 1).pow(2))));
                }
            }),
